...
class DiffusionEGNN(nn.Module):
    def __init__(self, number_tokens = 21, dim = 32, num_layers=4, in_features=16, hidden_features=32, T=1000):
    ...
    def forward(self, h, noise_x, node_mask):
        # 利用 EGNN 模型预测特征部分的噪声
        predicted_noise = self.egnn(h, noise_x, node_mask)
